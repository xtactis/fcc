...
void printAVR(const std::vector<u32> &instructions) {
    ...
    for (u32 instruction: instructions) {
        if ((instruction & 0xFC0F0000) == 0x90000000) {
            // LDS/STS - 32-bit instruction
            const u16 SRAMaddress = instruction & 0xFFFF;
            const u16 reg = (instruction & 0x003F00000) >> 16;
            if (instruction & 0x02000000) {
                // STS
                printf("[0x%08x]\tsts r%d, 0x%x\n", instruction, reg, SRAMaddress << 1);
            } else {
                // LDS
                printf("[0x%08x]\tlds 0x%x, r%d\n", instruction, SRAMaddress << 1, reg);
            }
            continue;
        }
        if ((instruction & 0xFE0C0000) == 0x940C0000) {
            // JMP/CALL - 32-bit instruction
            const u32 SRAMaddress = (instruction & 0x1FFFF) | ((instruction & 0x1F00000) >> 3);
            if (instruction & 0x00020000) {
                // CALL
                printf("[0x%08x]\tcall 0x%x\n", instruction, SRAMaddress << 1);
            } else {
                // JMP
        ...
        if (instruction == 0) {
            ...
            // BRBC/BRBS
            const s8 offset = (instruction & 0x03F8) >> 3;
            const u8 bit = (instruction & 0x7);
            if (instruction & 0x0400) {
                printf("[0x%04x]\tbrbs %u, %d\n", instruction, bit, offset);
            } else {
                printf("[0x%04x]\tbrbc %u, %d\n", instruction, bit, offset);
            }
